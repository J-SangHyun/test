...
function movePiece(i1, j1, i2, j2, d) {
  ...
  for(let i = -1; i < 2; i++) {
    for(let j = -1; j < 2; j++) {
      xpos = Math.min(Math.max(i2 + i, 0), BOARD_SIZE-1);
      ypos = Math.min(Math.max(j2 + j, 0), BOARD_SIZE-1);
...
function render_board() {
  ...
  const blockSize = Math.min(width, height) / (BOARD_SIZE + 1);
  ...
  const xPad = (width - blockSize * BOARD_SIZE) / 2;
  const yPad = (height - blockSize * BOARD_SIZE) / 2;
  for(let i = 0; i < BOARD_SIZE; i++) {
    for(let j = 0; j < BOARD_SIZE; j++) {
      rects[i][j] = new PIXI.Graphics();
      rects[i][j].lineStyle({width: 4, color: 0x000000, alpha: 1});
      rects[i][j].beginFill(isToggled(i, j) ? playerToggle[player-1] : 0xFFFFFF);
      rects[i][j].drawRect(0, 0, blockSize, blockSize);
      rects[i][j].endFill();
      rects[i][j].x = xPad + i * blockSize;
      rects[i][j].y = yPad + j * blockSize;
      rects[i][j].eventMode = 'static';
      rects[i][j].on('pointerdown', (event) => { clickBlock(i, j); });
      app.stage.addChild(rects[i][j]);
    }
  }
